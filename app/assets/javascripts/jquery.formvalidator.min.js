...
(function($) {
    ...
    var _applyErrorStyle = function($elem, conf) {
        ...
        _setInlineErrorMessage = function($input, mess, conf, $messageContainer) {
            ...
            if (custom) {
                ...
                if ($mess.length == 0) {
                    $mess = $("<label></label>").addClass("help-block").addClass(conf.errorMessageClass);
        ...
        this.find("textarea,input").each(function() {
            ...
            if (help) {
                $elem.addClass("has-help-txt").unbind("focus.help").bind("focus.help", function() {
                    ...
                    if ($help.length == 0) {
                        $help = $("<label />").addClass(className).addClass("help").addClass("help-block").text(help).hide();
                        $elem.after($help)
                    }
                    $help.fadeIn()
        ...
        suggest: function($elem, suggestions, settings) {
            ...
            $elem.data("suggestions", suggestions).valAttr("suggestion-nr", this._numSuggestionElements).unbind("focus.suggest").bind("focus.suggest", function() {
                ...
                var code = e.keyCode ? e.keyCode : e.which,
                    suggestionId, $suggestionContainer, $input = $(this);
                if (code == 13 && $.formUtils._selectedSuggestion !== null) {
                    ...
                    if ($suggestions.length > 0 && $.inArray(code, [38, 40]) > -1) {
                        if (code == 38) {
                            if ($.formUtils._selectedSuggestion === null) $.formUtils._selectedSuggestion = $suggestions.length - 1;
                            else $.formUtils._selectedSuggestion--;
                            if ($.formUtils._selectedSuggestion < 0) $.formUtils._selectedSuggestion = $suggestions.length - 1
                        } else if (code == 40) {
                            if ($.formUtils._selectedSuggestion === null) $.formUtils._selectedSuggestion = 0;
                            else $.formUtils._selectedSuggestion++;
    ...
    // $.formUtils.addValidator({    
    //     name: "equalTo"
    //         validatorFunction: function( val, element, param ) {
    //         // bind to the blur event of the target in order to revalidate whenever the target field is updated
    //         // TODO find a way to bind the event just once, avoiding the unbind-rebind overhead
    //         var target = $( param );
    //         if ( this.settings.onfocusout ) {
